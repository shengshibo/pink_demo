(function (window) {
  ...
  /*
    transition的问题
      1. 元素没有渲染完成时，无法触发过渡
      2. 在transform切换下，如果前后transform属性值变换函数的位置个数不一样，无法触发过渡
      3. 我们没有办法得到transition中任何一帧的状态
        ---- Tween算法
  */
  window.mozhi.vMove = function (wrap, callBack) {
    // 滑屏元素
    var item = wrap.children[0]
    mozhi.css(item, 'translateZ', 0)
    ...
    wrap.addEventListener('touchstart', function (ev) {
      ...
      element.y = mozhi.css(item, 'translateY')
      element.x = mozhi.css(item, 'translateZ')
    ...
    wrap.addEventListener('touchmove', function (ev) {
      ...
      // var nowPoint = mozhi.css(item, 'translateY')
      timeDis = nowTime - lastTime
      pointDis = nowPoint - lastPoint
      ...
      /*
        ...
        translateY = mozhi.css(item, 'translateY') + pointDis * scale
      */
      if (translateY > 0) {
        item.handMove = true
      ...
      mozhi.css(item, 'translateY', translateY)
    ...
    wrap.addEventListener('touchend', function () {
      var translateY = mozhi.css(item, 'translateY')
      if (!item.handMove) {
        // 速度越大 位移越远
        var speed = pointDis / timeDis
        speed = Math.abs(speed) < 0.5 ? 0 : speed
    ...
    function bsr (type, targetY, time) {
      ...
      var t = 0
      // 初始位置
      var b = mozhi.css(item, 'translateY')
      // 最终位置 - 初始位置
      var c = targetY - b
      // 总次数
      ...
      clearId = setInterval(function () {
        ...
        mozhi.css(item, 'translateY', point)
